...
本项目实现了一个高性能的手机号存在性判断系统，专为处理 **20 亿级别**手机号数据而设计，提供多种存储方案以适应不同的内存、精确性和性能需求。

**核心亮点**：MixTree 混合树方案仅需 **6.5MB** 内存即可实现百亿手机号存在性查询，利用真实运营商号段分布特性，将空间效率推向极致。

## 支持的方案

| 方案 | 命令行参数 | 说明 |
|------|-----------|------|
| Roaring Bitmap | `-t bitmap` | 内存极低，精确查询，速度极快 |
| Bloom Filter | `-t bloom` | 内存最低，有假阳性，速度极快 |
| SQLite | `-t sqlite` | 磁盘存储，精确查询，速度较慢 |
| Radix Tree | `-t tree` | 前缀共享，精确查询，内存较高 |
| MixTree | `-t mixtree` | **推荐**，前缀树+位图混合，空间效率最优 |
...
// 创建检查器
// typ: "bitmap" | "bloom" | "sqlite" | "tree" | "mixtree"
...
| 指标 | Bitmap | Bloom Filter | SQLite | Radix Tree | MixTree |
|------|--------|--------------|--------|------------|---------|
| 20亿内存 | ~800MB | ~230MB | <10MB | ~20GB | ~12.6GB |
| 20亿磁盘 | ~300MB | ~100MB | ~25GB | ~25GB | ~12.6GB |
| 查询时间 | 纳秒级 | 纳秒级 | 毫秒级 | 纳秒级 | 纳秒级 |
| 精确性 | 100% 精确 | 有误判 | 100% 精确 | 100% 精确 | 100% 精确 |
| 加载速度 | 秒级 | 秒级 | 立即 | 分钟级 | 分钟级 |
| 并发安全 | 只读安全 | 只读安全 | 是 | 只读安全 | 只读安全 |
| 需要CGO | 否 | 否 | 是 | 否 | 否 |
| 前缀统计 | ❌ | ❌ | ⚠️ 慢SQL | ✅ | ✅ |
...
| 场景 | 推荐方案 | 原因 |
|------|---------|------|
| 高并发实时查询，要求精确 | **MixTree / Bitmap** | 查询极快，100%精确 |
| 需要号段前缀统计功能 | **MixTree** | 内置前缀计数，O(1)响应 |
| 内存极度受限，可接受假阳性 | **Bloom Filter** | 内存占用最低 |
| 数据量远超内存，查询频率低 | **SQLite** | 磁盘存储，精确但较慢 |
| 数据量小 (<1000万) | 任意 | 差异不明显 |
| 极端内存受限 + 必须精确 | **MixTree (真实号段)** | 实际仅需6.5MB |
...
| 数据量 | Bitmap 内存 | Bloom Filter 内存 (1% fp) | MixTree 内存 (分散) | MixTree 内存 (真实号段) |
|--------|------------|-------------------------|---------------------|----------------------|
| 1000万 | ~4MB | ~1.2MB | ~63MB | ~6.5MB |
| 1亿 | ~40MB | ~12MB | ~630MB | ~6.5MB |
| 10亿 | ~400MB | ~115MB | ~6.3GB | ~6.5MB |
| 20亿 | ~800MB | ~230MB | ~12.6GB | ~6.5MB |
| 50亿 | ~2GB | ~575MB | ~31.5GB | ~6.5MB |
